...
{
    public partial class UtilitesWater : Form
    {
        RequestSQL requestSQL = new RequestSQL();

        int accessLevel;
        string nameTable = "ПоказанияВоды";
        string FieldCode = "КодПоказанияВоды";
        string[] header = { "Код Показания Воды", "Стоимость КубМ", "Показания На Начало Месяца", "Показания На Конец Месяца", "КодПомещения" };
        ...
        {
            requestSQL.LoadDataGrid(ref dataGridViewUtilitesWater, nameTable, header);
        }
        ...
        public UtilitesWater(int AccessLevel)
        {
            InitializeComponent();
            accessLevel = AccessLevel;
        }

        private void UtilitesWater_Load(object sender, EventArgs e)
        {
            DataGridUpdate();
            ComboBoxLoad();
        }

        private void dataGridViewUtilitesWater_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex != -1)
            {
                textBoxIndication.Text = dataGridViewUtilitesWater.CurrentRow.Cells[0].Value.ToString();
                textBoxStartMonth.Text = dataGridViewUtilitesWater.CurrentRow.Cells[2].Value.ToString();
                textBoxEndMonth.Text = dataGridViewUtilitesWater.CurrentRow.Cells[3].Value.ToString();
                textBoxPrice.Text = dataGridViewUtilitesWater.CurrentRow.Cells[1].Value.ToString();
                comboBoxCodeRoom.Text = dataGridViewUtilitesWater.CurrentRow.Cells[4].Value.ToString();
            }
        }
        ...
        private void buttonSearch_Click(object sender, EventArgs e)
        {
            ...
            requestSQL.SearchSQL(ref dataGridViewUtilitesWater, header, nameTable, FieldCode, code);
        }
